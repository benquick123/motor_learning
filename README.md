...
- `desired_trial_time`: an array determining the interval of the desired time for the completion of the movement. Only used in the trial_mode branch. In seconds.
- `desired_velocity`: an array determining the minimum and maxumum desired velocity during a trial. Only used in the trial_mode branch. In m/s.
...
Variable "velocity_buffer_size" determines the size of velocity buffer. Velocity buffer is used to smoothen the perturbation forces and calculate the maximum velocity of a trial to determine whether is was (un)successful.